...
agent: auto  # auto | claude | copilot | codex | rovodev | opencode | pi | acp:<target>
...
- Global `agent: auto` resolves by checking `claude`, `copilot`, `codex`, `opencode`, `acli` for `rovodev`, then `pi` on `PATH`.
